...
{   
    int crossResult[3] = {0,0,0};
    for(int i=0;i<3;i++)
        crossResult[i] = ((x-_v[i].x())*(_v[(i+1)%3].y()-_v[i].y()))-((y-_v[i].y())*(_v[(i+1)%3].x()-_v[i].x()));
    if((crossResult[0]>=0 && crossResult[1]>=0 && crossResult[2]>=0) || (crossResult[0]<=0 && crossResult[1]<=0 && crossResult[2]<=0))
        return true;
    else
        return false;
}
